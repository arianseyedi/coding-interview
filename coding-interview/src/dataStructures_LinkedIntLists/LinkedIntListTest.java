 ...
 * 
 * @author York University Computer Science Department Course: EECS2030,
 *         Fall2017
 *
 */
@FixMethodOrder(MethodSorters.NAME_ASCENDING) // test01 before test02 etc..
public class LinkedIntListTest {
	...
	private void checkStructure(List<Integer> exp, LinkedIntList got) {
		if (exp.isEmpty()) {
			...
			if (exp.size() != got.size()) {
				String err = String.format("list has the wrong size; expected size: %d, got size: %d", exp.size(),
						got.size());
				fail(err);
			}
			Node n = got.head();
			for (int i = 0; i < exp.size(); i++) {
				...
				if (expData != gotData) {
					String err = String.format("node: %d has the wrong data; expected: %d but got: %d", i, expData,
							gotData);
					fail(err);
				}
				if (i == exp.size() - 1) {
	...
	/**
	 * Tests that there are exactly 3 fields and that the types of the fields are
	 * either int or Node.
	 */
	@Test
	...
	/**
	 * Tests the getNode method; includes testing of the expected exception.
	 */
	@Test
	public void test04_getNode() {
		...
		for (int i = 0; i < 10; i++) {
			try {
				t.getNode(i);
				fail(String.format("getNode(%d) should have thrown an exception for list: %s", i, exp));
			} catch (IndexOutOfBoundsException x) {
				// ok
			} catch (Exception x) {
				fail(String.format("getNode(%d) threw an unexpected exception of type %s", i, x.getClass().getName()));
			}
			int val = rng.nextInt(101) - 50;
			exp.add(val);
			t.add(val);
			for (int j = 0; j <= i; j++) {
				...
				assertEquals(String.format("getNode(%d) failed for list: %s", j, exp.toString()), expVal, gotVal);
				checkStructure(exp, t);
			}
			try {
				t.getNode(i + 1);
				fail(String.format("getNode(%d) should have thrown an exception for list: %s", i + 1, exp));
			} catch (IndexOutOfBoundsException x) {
				// ok
			} catch (Exception x) {
				fail(String.format("getNode(%d) threw an unexpected exception of type %s", i + 1,
						x.getClass().getName()));
			}
		}
	}
	...
	/**
	 * Tests the get method; includes testing of the expected exception.
	 */
	@Test
	public void test05_get() {
		...
		for (int i = 0; i < 10; i++) {
			try {
				t.get(i);
				fail(String.format("get(%d) should have thrown an exception for list: %s", i, exp));
			} catch (IndexOutOfBoundsException x) {
				// ok
			} catch (Exception x) {
				fail(String.format("get(%d) threw an unexpected exception of type %s", i, x.getClass().getName()));
			}
			int val = rng.nextInt(101) - 50;
			exp.add(val);
			t.add(val);
			for (int j = 0; j <= i; j++) {
				int expVal = exp.get(j);
				int gotVal = t.get(j);
				assertEquals(String.format("get(%d) failed for list: %s", j, exp.toString()), expVal, gotVal);
				checkStructure(exp, t);
			}
			try {
				t.get(i + 1);
				fail(String.format("get(%d) should have thrown an exception for list: %s", i + 1, exp));
			} catch (IndexOutOfBoundsException x) {
				// ok
			} catch (Exception x) {
				fail(String.format("get(%d) threw an unexpected exception of type %s", i + 1, x.getClass().getName()));
			}
		}
	}

	/**
	 * Tests the set method; includes testing of the expected exception.
	 */
	@Test
	public void test06_set() {
		...
		for (int i = 0; i < 10; i++) {
			try {
				t.set(i, 0);
				fail(String.format("set(%d, 0) should have thrown an exception for list: %s", i, exp));
			} catch (IndexOutOfBoundsException x) {
				// ok
			} catch (Exception x) {
				fail(String.format("set(%d, 0) threw an unexpected exception of type %s", i, x.getClass().getName()));
			}
			exp.add(-100);
			t.add(-100);
			for (int j = 0; j <= i; j++) {
				...
				int expVal = exp.get(j);
				int gotVal = t.get(j);
				assertEquals(String.format("set(%d, %d) failed for list: %s", j, val, exp.toString()), expVal, gotVal);
				assertEquals(String.format("set(%d, %d) returned the wrong value", j, val), expReturn, gotReturn);
				checkStructure(exp, t);
			}
			try {
				t.set(i + 1, 0);
				fail(String.format("set(%d, 0) should have thrown an exception for list: %s", i + 1, exp));
			} catch (IndexOutOfBoundsException x) {
				// ok
			} catch (Exception x) {
				fail(String.format("set(%d, 0) threw an unexpected exception of type %s", i + 1,
						x.getClass().getName()));
			}
		}
	}
	...
	/**
	 * Tests the removeFirst method; includes testing of the expected exception.
	 */
	@Test
	public void test12_removeFirst() {
		...
		try {
			...
			fail(String.format("removeFirst() threw an unexpected exception of type %s", x.getClass().getName()));
		}
		...
		for (int i = 0; i < 10; i++) {
			...
			checkStructure(exp, t);
			assertEquals("removeFirst() returned the wrong value", expReturn, gotReturn);

		}
	}

	/**
	 * Tests the removeLast method; includes testing of the expected exception.
	 */
	@Test
	public void test13_removeLast() {
		...
		try {
			...
			fail(String.format("removeLast() threw an unexpected exception of type %s", x.getClass().getName()));
		}
		...
		for (int i = 0; i < 10; i++) {
			...
			checkStructure(exp, t);
			assertEquals("removeLast() returned the wrong value", expReturn, gotReturn);
	...
	public void test14_remove() {
		...
		try {
			...
			fail(String.format("remove(0) threw an unexpected exception of type %s", x.getClass().getName()));
		}
	}
	...
	public void test15_remove() {
		...
		try {
			...
			fail(String.format("remove(4) threw an unexpected exception of type %s", x.getClass().getName()));
		}
	}
	...
	public void test17_remove() {
		...
		for (int i = 0; i < 10; i++) {
			...
			checkStructure(exp, t);
			assertEquals(String.format("remove(%d) returned the wrong value", idx), expReturn, gotReturn);
	...
	public void test18_shiftRight() {
		...
		final Integer[][] EXP = { { 0, 1, 2, 3, 4 }, { 4, 0, 1, 2, 3 }, { 3, 4, 0, 1, 2 }, { 2, 3, 4, 0, 1 },
				{ 1, 2, 3, 4, 0 }, { 0, 1, 2, 3, 4 } };
